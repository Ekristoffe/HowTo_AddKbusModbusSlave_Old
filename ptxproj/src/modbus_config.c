...
{
   size_t termCnt;
   uint16_t termInfo[MODBUSCONFIG_MAX_TERMINALS_1 + MODBUSCONFIG_MAX_TERMINALS_2 + MODBUSCONFIG_MAX_TERMINALS_3 + MODBUSCONFIG_MAX_TERMINALS_4];

   if (kbus_getTerminals(&termCnt, termInfo, sizeof(termInfo)/sizeof(uint16_t)))
       return -1;

   mb_knot_asam_1->tab_registers[0] = conf_order_number; //Fake 750-xxxx Settings in the kbusmodbusslave.conf file
   size_t i=0;
   for (i = 1; i <= termCnt; i++)
   {
       if ((i >= 1) && (i <=64))
