		...
		case _FC_READ_COILS: 
			{
				...
				if (nb < 1 || MODBUS_MAX_READ_BITS < nb) 
				{
					if (ctx->debug) 
					{
						fprintf(stderr, "Illegal nb of values %d in read_bits (max %d)\n", nb, MODBUS_MAX_READ_BITS);
					}
				...
				else 
				{
					...
					rsp_length += modbus_get_bytes_from_bitmap16(mb_mapping->tab_bits, address, nb, &rsp[rsp_length]);
				}
			}
			break;
		case _FC_READ_DISCRETE_INPUTS: 
			{
				...
				if (nb < 1 || MODBUS_MAX_READ_BITS < nb) 
				{
					if (ctx->debug) 
					{
						fprintf(stderr, "Illegal nb of values %d in read_input_bits (max %d)\n", nb, MODBUS_MAX_READ_BITS);
					}
					rsp_length = response_exception(ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp);
				} 
				...
				else 
				{
					rsp_length = ctx->backend->build_response_basis(&sft, rsp);
					rsp[rsp_length++] = (nb / 8) + ((nb % 8) ? 1 : 0);
					rsp_length += modbus_get_bytes_from_bitmap16(mb_mapping->tab_input_bits, address, nb, &rsp[rsp_length]);
				}
			}
			break;
		...
		case _FC_READ_INPUT_REGISTERS: 
			{
				...
				if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) 
				{
					if (ctx->debug) 
					{
						fprintf(stderr, "Illegal number of values %d in read_input_registers (max %d)\n", nb, MODBUS_MAX_READ_REGISTERS);
					}
					rsp_length = response_exception(ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp);
				} 
		...
		case _FC_WRITE_SINGLE_COIL:
			{
				...
				else 
				{
					...
					else 
					{
						if (ctx->debug) 
						{
							fprintf(stderr, "Illegal data value %0X in write_bit request at address %0X\n", data, address);
						}
						rsp_length = response_exception(ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp);
					}
				}
		...
		case _FC_WRITE_AND_READ_REGISTERS: 
			{
				...
				if (nb_write < 1 || MODBUS_MAX_RW_WRITE_REGISTERS < nb_write || nb < 1 || MODBUS_MAX_READ_REGISTERS < nb || nb_write_bytes != nb_write * 2) 
				{
					if (ctx->debug) 
					{
						fprintf(stderr, "Illegal nb of values (W%d, R%d) in write_and_read_registers (max W%d, R%d)\n", nb_write, nb, MODBUS_MAX_RW_WRITE_REGISTERS, MODBUS_MAX_READ_REGISTERS);
					}
					rsp_length = response_exception(ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp);
				} 
				else 
				{
					...
					else
					{
						...
						{
							write_mapping->tab_registers[i] = (req[offset + j] << 8) + req[offset + j + 1];
						}
		...
		case _FC_READ_INPUT_REGISTERS_XL: 
			{
				...
				if (nb < 1 || MODBUS_MAX_READ_REGISTERS_FC66 < nb) 
				{
					if (ctx->debug) 
					{
						fprintf(stderr, "Illegal number of values %d in read_input_registers_xl (max %d)\n", nb, MODBUS_MAX_READ_REGISTERS);
					}
					rsp_length = response_exception(ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp);
				} 
				else if ((address + nb) > mb_mapping->nb_input_registers) 
